        ...
        <div className="w-full h-full border border-red-500 flex gap-2">
            <div className='w-14 h-full flex flex-col gap-4 mt-2 border-r border-r-dividerDark'>
                <Tooltip position='side-left' text='Collections'>
                    <CiFolderOn size={18} className={`cursor-pointer ${selectedTab === 'Collections' ? 'text-accentDark' : 'text-secondaryLight'}`}
                        onClick={() => setSelectedTab('Collections')} />
                </Tooltip>
                <Tooltip position='side-left' text='Environments'>
                    <IoLayersOutline size={18} className={`cursor-pointer ${selectedTab === 'Environments' ? 'text-accentDark' : 'text-secondaryLight'}`}
                        onClick={() => setSelectedTab('Environments')} />
                </Tooltip>
                <Tooltip position='side-left' text='History'>
                    <FaRegClock size={18} className={`cursor-pointer ${selectedTab === 'History' ? 'text-accentDark' : 'text-secondaryLight'}`}
                        onClick={() => setSelectedTab('History')} />
                </Tooltip>
                <Tooltip position='side-left' text='Shared Requests'>
                    <IoShareSocialOutline size={18} className={`cursor-pointer ${selectedTab === 'Shared Requests' ? 'text-accentDark' : 'text-secondaryLight'}`}
                        onClick={() => setSelectedTab('Shared Requests')} />
                </Tooltip>
                <Tooltip position='side-left' text='Generate Code'>
                    <IoCode size={18} className={`cursor-pointer ${selectedTab === 'Generate Code' ? 'text-accentDark' : 'text-secondaryLight'}`}
                        onClick={() => setSelectedTab('Generate Code')} />
                </Tooltip>

            </div>
            <div className='w-full h-full flex flex-col gap-1 '>
                <div className="flex p-2 gap-1 justify-start align-middle text-secondaryLight border-b border-b-dividerDark">
